...
public class SkeletonChase : MonoBehaviour {
        ...
        if (hitColliders.Length != 0)
        {
            ...
            while (i < hitColliders.Length)
            {
                ...
                if (hitColliders[i].gameObject.tag == "Terrain" || hitColliders[i].gameObject.name == this.name || hitColliders[i].gameObject.tag == this.tag)//GameObject is self, or terrain 
                                                                                                                                                              //or its tag(type) is self. Only Further Check NPC's or Player.
                {
            ...
            //int f = 0;
            //while (f < hitColliders.Length) { Debug.Log(hitColliders[f].gameObject); f++; };
    ...
    {
        if (target != null && !damaged)//If target is null idle and not damaged.
        {
